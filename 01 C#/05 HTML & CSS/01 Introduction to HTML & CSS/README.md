...
#### Class selectors
Може дефинираме клас в CSS, използвайки име с точка (`.`) пред него, и след това да приложим този клас към всеки HTML елемент чрез атрибута `class`. Например, ако дефинираме клас, наречен `odd`:
