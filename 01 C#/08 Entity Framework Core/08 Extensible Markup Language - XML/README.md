...
За да запишем `XDocument` в файл със стандартни настройки:

```csharp
xmlDoc.Save("myBooks.xml");
```

За да деактивираме автоматичното индентация:

```csharp
xmlDoc.Save("myBooks.xml", SaveOptions.DisableFormatting);
```

За сериализиране на обект в XML и записването му в файл:
...
- **`Save("myBooks.xml")`**: Записва XML документа във файл с използване на стандартни настройки, като автоматично добавя индентация и нови редове за по-добра четимост.
- **`Save("myBooks.xml", SaveOptions.DisableFormatting)`**: Записва XML документа във файл, като деактивира автоматичното форматиране (индентация), което прави резултата по-компактен.
- **`XmlSerializer.Serialize()`**: Сериализира даден обект в XML формат и го записва в указания файл чрез `StreamWriter`.
...
Алтернативен начин с използване на `XmlRootAttribute`:
