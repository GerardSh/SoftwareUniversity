...
### Repository
**Repository** е слой, който се грижи за достъпа до базата данни и за попълването на обектите с данни. Това е единственото място в приложението, което има достъп до базата данни, и неговата задача е да извършва CRUD операциите — създаване (Create), четене (Read), актуализиране (Update) и изтриване (Delete).

Репозиторито няма представа за какво ще се ползват данните, нито каква е тяхната бизнес логика. Неговата роля е единствено да знае:

- Достъпва данните
- Добавя нови данни
- Променя съществуващи данни
- Изтрива ненужни данни

Domain ентититата, логиката за достъп до данни и бизнес логиката комуникират чрез интерфейси, което осигурява по-добра изолация и гъвкавост в архитектурата.

Основното предимство на този подход е, че скрива детайлите от достъпа до базата данни — приложението не е необходимо да знае как точно се осъществява достъпът.

В практиката:

- Без репозиторита: бизнес логиката съдържа и логика за достъп до данни, което води до тясна свързаност между слоевете.
- С репозиторита: бизнес логиката, логиката за достъп до данни и domain ентититата са ясно разграничени, което подобрява поддръжката и разширяемостта на кода.
### Unit of Work
Има една основна цел – да гарантира, че когато ползваме множество репозиторита, те споделят един и същ контекст за достъп до базата данни.

С помощта на **Unit of Work** може също да добавим функционалност за **Отмяна / Връщане на промени** (Undo / Rollback).

При работа с **Entity Framework Core** препоръчителният подход за отмяна на промените е да отхвърлим текущия контекст, съдържащ нежеланите промени. Например, ако сме започнали да правим промени в контекста и все още не сме извикали `SaveChanges()`, промените не са запазени в базата данни. Ако на този етап решим, че сме допуснали грешка или промените не са желани, най-чистият вариант е просто да изхвърлим текущия контекст и да създадем нов.

Когато ползваме `SaveChanges()` в EF Core, всички извършени промени се записват в базата данни едновременно.
# Misc
# ChatGPT
# Bookmarks
