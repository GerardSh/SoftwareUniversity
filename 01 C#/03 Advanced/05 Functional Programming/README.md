...
![](Pasted%20image%2020240425171446.png)

При декларирането на функцията, първите типове данни са какви аргументи трябва да се подадат, а последния тип е какво трябва да връща. В примера горе, функцията приема два int-a и връща long. Понеже имат еднакъв signature с метода Sum, то няма проблем да присвоим като стойност на функцията `funcMethod`, метода `Sum`. Можем също и директно да зададем тялото на функцията видно от примера. 

`Func<int, int, long>` - Тип данни във програмирането, който отговаря на дадена функция, наричаме делегат, тоест самото описание / signature-a на функцията.
С други думи, делегатите в C# представляват типове, които описват сигнатурата на метод. Те се използват за представяне на методи като обекти и позволяват гъвкаво използване на функции като параметри, резултати и други.
Можем да си направим собствен делегат, тоест декларираме тип данни, който описва функции, това е полезно, ако искаме да дадем име на конкретен тип функции:
...
Една от ползите на `Func<>` е че може да присвояваме нови стойности, без да се налага да създаваме отделни методи. Примерно вместо да създаваме два отделни метода, които приемат две числа и връщат резултата, като в първия метод ще търсим техния сбор а във втория ще ги умножим, може да имаме само една функция и динамично да й променяме тялото видно от примера горе.
...
![](Pasted%20image%2020240425120628.png)

Това е равносилно като да дефинираме `Func<>` който е равен  на `int.Parse`, след което функцията selector, ще бъде равна на метода `int.Parse()`.
...
# ChatGPT
## Delegates
Може вместо да пишем lambda израз, да подадем друг метод, който има същия signature, като `Func<>` в метода. Примерно: ``` string numbers = "1 2 3 4"; int[] numbersArray = numbers.Split().Select(int.Parse).ToArray(); int number = int.Parse("5"); ``` Може да видим че `Select()` метода очаква метод със signature приемащ string стойност и връщащ int стойност. Точно такъв метод е `int.Parse()` и затова може да го подадем на функцията която е параметър с името selector в метода `Select()`.
