...
Можем да деструктурираме един обект в друг и да презапишем стойности. Това става чрез оператора за деструктуриране и разпръскващия оператор. Пример:

```javascript
const original = { name: "John", age: 30, city: "New York" };
const updated = { ...original, age: 35 }; // Презаписваме age

console.log(updated); // { name: "John", age: 35, city: "New York" }
```

- `...original` копира всички пропъртита от оригиналния обект.
- `age: 35` презаписва стойността на `age` в новия обект.
