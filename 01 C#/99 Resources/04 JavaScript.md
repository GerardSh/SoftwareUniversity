...
`Promise.allSettled(iterable)` – Връща обещание. Изпълнява се, когато всички обещания в подадената колекция приключат, независимо дали са изпълнени успешно, или са отхвърлени. Методът гарантира, че ще се обработят всички резултати, като за всеки обещан обект ще бъде върнат обект със статус (успешен или неуспешен) и съответния резултат или грешка. Това е полезно, когато искаме да изчакаме всички асинхронни операции да завършат, но не се притесняваме дали те ще са успешни или не.

`Promise.race(iterable)` – Връща обещание, което се изпълнява веднага щом първото обещание от подадената колекция бъде изпълнено или отхвърлено. Това означава, че ако имаме множество асинхронни операции, ще получим резултата или грешката от първата завършена операция, без значение дали останалите все още изпълняват. Примерно ако имаме три сървъра и искаме да пуснем заявки към тях паралелно и да вземем отговор от първия завършен сървър.
## Misc
